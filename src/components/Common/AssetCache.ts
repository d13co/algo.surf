...
function runTx<T>(mode: IDBTransactionMode, runner: (store: IDBObjectStore) => Promise<T> | T): Promise<T> {
	return openDB().then(
		(db) =>
...
export const AssetCache = {
	...
	async searchByNameOrUnit(partial: string, limit: number = 100): Promise<A_AssetTiny[]> {
		...
		return runTx('readonly', async (store) => {
			...
			const scanIndex = (indexName: 'name' | 'unitName') =>
				new Promise<void>((resolve, reject) => {
					...
					req.onsuccess = () => {
						...
						const field = (indexName === 'name' ? value.name : value.unitName) || '';
			...
			return results.slice(0, limit).map(fromRow);
		});
	},
};
