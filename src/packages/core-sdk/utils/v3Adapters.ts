...
function convertAssetFreeze(
  ...
  return {
    ...
    "asset-id": getNumber(obj, ["asset-id", "assetId"]),
    "new-freeze-status": getBoolean(obj, [
      "new-freeze-status",
      "newFreezeStatus",
    ]),
  };
...
function convertAppCall(
  obj: unknown
): A_SearchTransaction_App_Call_Payload | undefined {
  if (!isObject(obj)) return undefined;

  const accounts = getArray(obj, ["accounts"]).map((v) => toString(v));
  const applicationArgs = getArray(obj, [
    "application-args",
    "applicationArgs",
  ]).map((v) => toString(v));
  const foreignApps = getArray(obj, ["foreign-apps", "foreignApps"]).map((v) =>
    toNumber(v)
  );
  const foreignAssets = getArray(obj, ["foreign-assets", "foreignAssets"]).map(
    (v) => toNumber(v)
  );
  ...
  return {
    ...
    "clear-state-program": getString(obj, [
      "clear-state-program",
      "clearStateProgram",
    ]),
    ...
    "global-state-schema":
      isObject(obj["global-state-schema"]) || isObject(obj["globalStateSchema"])
        ? (serializeValue(
            obj["global-state-schema"] ?? obj["globalStateSchema"]
          ) as A_SearchTransaction_App_Call_Payload["global-state-schema"])
        : { "num-byte-slice": 0, "num-uint": 0 },
    "local-state-schema":
      isObject(obj["local-state-schema"]) || isObject(obj["localStateSchema"])
        ? (serializeValue(
            obj["local-state-schema"] ?? obj["localStateSchema"]
          ) as A_SearchTransaction_App_Call_Payload["local-state-schema"])
        : { "num-byte-slice": 0, "num-uint": 0 },
    "on-completion": getString(obj, ["on-completion", "onCompletion"]),
  };
}

function convertKeyReg(
  obj: unknown
): A_SearchTransaction_KeyReg_Payload | undefined {
  if (!isObject(obj)) return undefined;
  return {
    "non-participation": getBoolean(obj, [
      "non-participation",
      "nonParticipation",
    ]),
    "selection-participation-key": getString(obj, [
      "selection-participation-key",
      "selectionParticipationKey",
    ]),
    "vote-first-valid": getNumber(obj, ["vote-first-valid", "voteFirstValid"]),
    "vote-key-dilution": getNumber(obj, [
      "vote-key-dilution",
      "voteKeyDilution",
    ]),
    "vote-last-valid": getNumber(obj, ["vote-last-valid", "voteLastValid"]),
    "vote-participation-key": getString(obj, [
...
function convertHeartbeat(
  obj: unknown
): A_SearchTransaction_Heartbeat_Payload | undefined {
  if (!isObject(obj)) return undefined;
  ...
  return {
    multisig: isObject(multisig)
      ? {
          ...
          subsignature: (
            getArray(multisig, ["subsignature"]) as JsonObject[]
          ).map((s) => ({
...
function convertStateDelta(obj: unknown): A_GlobalStateDelta[] | undefined {
  ...
  return obj.filter(isObject).map(
    (x): A_GlobalStateDelta => ({
      key: getString(x, ["key"]),
      value: {
        action: getNumber(x["value"] as JsonObject, ["action"]),
        ...(x["value"]["bytes"] !== undefined
          ? { bytes: getString(x["value"] as JsonObject, ["bytes"]) }
          : {}),
        ...(x["value"]["uint"] !== undefined
...
function convertAssetConfig(obj: unknown): A_Asset | undefined {
  ...
  const params = isObject(paramsRaw)
    ? (serializeValue(paramsRaw) as A_Asset["params"])
    : undefined;
  return typeof index === "number" && params
    ? ({ index, params } as A_Asset)
    : undefined;
}

function convertStateProof(
  obj: unknown
): A_SearchTransaction_State_Proof_Payload | undefined {
  if (!isObject(obj)) return undefined;
...
export function toA_SearchTransaction(input: unknown): A_SearchTransaction {
  ...
  const innerRaw = getArray(o, ["inner-txns", "innerTxns"]).map(
    toA_SearchTransaction
  );
  ...
  const txn: A_SearchTransaction = {
    ...
    "intra-round-offset": getNumber(o, [
      "intra-round-offset",
      "intraRoundOffset",
    ]),
    ...
    "created-application-index": getNumber(o, [
      "created-application-index",
      "createdApplicationIndex",
    ]),
    "created-asset-index": getNumber(o, [
      "created-asset-index",
      "createdAssetIndex",
    ]),

    "application-transaction": convertAppCall(
      o["application-transaction"] ?? o["applicationTransaction"]
    ),
    "asset-transfer-transaction": convertAssetTransfer(
      o["asset-transfer-transaction"] ?? o["assetTransferTransaction"]
    ),
    "asset-freeze-transaction": convertAssetFreeze(
      o["asset-freeze-transaction"] ?? o["assetFreezeTransaction"]
    ),
    "payment-transaction": convertPayment(
      o["payment-transaction"] ?? o["paymentTransaction"]
    ),
    "asset-config-transaction": convertAssetConfig(
      o["asset-config-transaction"] ?? o["assetConfigTransaction"]
    ),
    "keyreg-transaction": convertKeyReg(
      o["keyreg-transaction"] ?? o["keyregTransaction"]
    ),
    "heartbeat-transaction": convertHeartbeat(
      o["heartbeat-transaction"] ?? o["heartbeatTransaction"]
    ),
    "state-proof-transaction": convertStateProof(
      o["state-proof-transaction"] ?? o["stateProofTransaction"]
    ),

    "global-state-delta": convertStateDelta(
      o["global-state-delta"] ?? o["globalStateDelta"]
    ),
    "local-state-delta": convertLocalStateDelta(
      o["local-state-delta"] ?? o["localStateDelta"]
    ),
...
export function toA_Block(input: unknown): A_Block {
  const o = isObject(input) ? input : {};
  const transactions = (getArray(o, ["transactions", "txns"]) as unknown[]).map(
    toA_SearchTransaction
  );
  const block: A_Block = {
    round: getNumber(o, ["round"]),
    timestamp: getNumber(o, ["timestamp", "time"], 0),
    "txn-counter": getNumber(
      o,
      ["txn-counter", "txnCounter", "transactionsCount"],
      transactions.length
    ),
    transactions,
...
export function toA_AccountsResponse(input: unknown): {
  ...
  const accounts: A_SearchAccount[] = accountsRaw.map(
    (a): A_SearchAccount => ({
      address: getString(a, ["address"]),
      amount: getNumber(a, ["amount"]),
      status: getString(a, ["status"]),
      "total-apps-opted-in": getNumber(a, [
        "total-apps-opted-in",
        "totalAppsOptedIn",
      ]),
      "total-assets-opted-in": getNumber(a, [
        "total-assets-opted-in",
        "totalAssetsOptedIn",
      ]),
      "total-created-apps": getNumber(a, [
        "total-created-apps",
        "totalCreatedApps",
      ]),
      "total-created-assets": getNumber(a, [
...
export function toA_Application(input: unknown): A_Application {
  ...
  const params = {
    "approval-program": getString(
      serializedParams,
      ["approval-program", "approvalProgram"],
      ""
    ),
    "clear-state-program": getString(
      serializedParams,
    ...
    "global-state":
      serializedParams["global-state"] ?? serializedParams["globalState"],
    "global-state-schema": convertSchema(
      serializedParams["global-state-schema"] ??
        serializedParams["globalStateSchema"]
    ),
    "local-state-schema": convertSchema(
      serializedParams["local-state-schema"] ??
        serializedParams["localStateSchema"]
    ),
    "extra-program-pages": getNumber(
      serializedParams,
      ["extra-program-pages", "extraProgramPages"],
      0
    ),
  };
...
export function toA_ApplicationsResponse(input: unknown): {
  "next-token": string;
...
export function toA_Asset(input: unknown): A_Asset {
  ...
  const params = {
    ...
    "default-frozen": getBoolean(
      serializedParams,
      ["default-frozen", "defaultFrozen"],
      false
    ),
    ...
    "metadata-hash": getString(serializedParams, [
      "metadata-hash",
      "metadataHash",
    ]),
  };
...
export function toA_Status(input: unknown): A_Status {
  ...
  return {
    ...
    "catchpoint-acquired-blocks": getNumber(
      o,
      ["catchpoint-acquired-blocks", "catchpointAcquiredBlocks"],
      0
    ),
    "catchpoint-processed-accounts": getNumber(
      o,
      ["catchpoint-processed-accounts", "catchpointProcessedAccounts"],
      0
    ),
    "catchpoint-total-accounts": getNumber(
      o,
      ["catchpoint-total-accounts", "catchpointTotalAccounts"],
      0
    ),
    "catchpoint-total-blocks": getNumber(
      o,
      ["catchpoint-total-blocks", "catchpointTotalBlocks"],
      0
    ),
    "catchpoint-verified-accounts": getNumber(
      o,
      ["catchpoint-verified-accounts", "catchpointVerifiedAccounts"],
      0
    ),
    ...
    "stopped-at-unsupported-round": ((): boolean => {
      const v =
        o["stopped-at-unsupported-round"] ?? o["stoppedAtUnsupportedRound"];
      return typeof v === "boolean" ? v : false;
    })(),
    "time-since-last-round": getNumber(
      o,
      ["time-since-last-round", "timeSinceLastRound"],
      0
    ),
  };
}
...
export function toA_Genesis(input: unknown): A_Genesis {
  if (typeof input === "string") {
    try {
      return JSON.parse(input) as A_Genesis;
    } catch {
      /* fallthrough */
    }
  }
